...
class App:
    ...
    def deck_init(self):
        """Read 'cards.json' and store its content in an instance variable"""
    ...
    def count_due_cards(self, deck):
        """Count all the cards that need to be reviewed"""
        c = 0
        for card in deck:
    ...
    def review1(self):
        ...
        study_now = self.find_dues()

        while self.study:
            if self.dues_exist(study_now):
    ...
    def find_dues(self):
        """Return a list containing all due cards and their indexes"""
        study_now = []
    ...
    def edit(self, card):
        front_or_back = input("[1] Edit front;\n"
                              ...
                              "[3] Cancel\n>>>.")
        if front_or_back == '1':
            front = input("Enter the data: ")
            card[1]["front"] = front
        elif front_or_back == '2':
            back = input("Enter the data: ")
            card[1]["back"] = back
        else:
            return

    def delete_marked(self):
        """Delete all cards marked with 'delete'"""
        rewrite = False
        
        for index, card in enumerate(self.cards["cards"]):
            if card["delete"]:
                rewrite = True
                self.cards["cards"].pop(index)

        if rewrite:
            self.rewrite(study_now=False)
